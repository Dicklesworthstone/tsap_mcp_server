"""
ToolAPI Client - Base Implementation

This module provides the base client class for interacting with TSAP's ToolAPI protocol.
"""
import uuid
import httpx
from typing import Dict, Any, Optional, List, Union

# Default ToolAPI server URL
DEFAULT_SERVER_URL = "http://localhost:8021"

class ToolAPIClient:
    """Client for interacting with the TSAP ToolAPI Server.
    
    This class provides methods for sending commands to a ToolAPI server
    ...
    def __init__(self, base_url: str = DEFAULT_SERVER_URL):
        """Initialize the ToolAPI client.
        
        Args:
            base_url: Base URL of the ToolAPI server,
                e.g., "http://localhost:8021"
        """
        # Ensure base_url has no trailing slash
        self.base_url = base_url.rstrip("/")
        
        # Set default API endpoint
        if not self.base_url.endswith("/toolapi"):
            self.api_endpoint = f"{self.base_url}/toolapi/"
        else:
            self.api_endpoint = f"{self.base_url}/"
    ...
    async def send_request(self, command: str, args: Dict[str, Any], mode: Optional[str] = None) -> Dict[str, Any]:
        """Send a ToolAPI request to the server.
        
        Args:
            command: The ToolAPI command name
            args: Command arguments
            mode: Optional performance mode (fast, standard, deep)
            
        Returns:
            ToolAPI response as a dictionary
        """
        # Create a ToolAPI request payload
        request = {
        ...
        # Add optional mode if specified
        if mode is not None:
            request["mode"] = mode
            
        # Send the request to the ToolAPI endpoint
        response = await self.client.post(self.api_endpoint, json=request)
    ...
    def extract_result(self, response: Dict[str, Any]) -> Dict[str, Any]:
        """Safely extract result data from a nested ToolAPI response.
        
        Handles the common nested data structure in ToolAPI responses:
        {
            "status": "success",
            "data": {
                "status": "success",
        ...
        Args:
            response: The ToolAPI response dictionary
    ...
    async def send_request_and_extract(self, command: str, args: Dict[str, Any], mode: Optional[str] = None) -> Dict[str, Any]:
        """Send a ToolAPI request and extract the result data in one step.
    ...
    async def jq_process(
        ...
        **kwargs
        ...
        # Send the request
        # The handler expects the command name defined in ToolAPICommandType
    ...
    async def sqlite_query(
        ...
        **kwargs
        ...
        """Execute a SQLite query via the ToolAPI server.
