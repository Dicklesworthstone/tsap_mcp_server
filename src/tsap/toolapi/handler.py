"""
ToolAPI protocol request handler.

This module provides handlers for processing ToolAPI protocol requests and
...
from tsap.core.table_processor import get_table_processor
from tsap.core.html_processor import process_html, HtmlProcessParams

from .protocol import (
    ToolAPIRequest, ToolAPIResponse, ToolAPICommandType,
...
def register_command_handler(command: str, handler: Callable):
    """Register a handler function for a ToolAPI command.
    ...
    key = command.value if isinstance(command, ToolAPICommandType) else command
    _command_handlers[key] = handler
    logger.debug(
        f"Registered handler for ToolAPI command: {key}", # Log the key being used
        component="toolapi",
...
def get_command_handler(command: str) -> Optional[Callable]:
    """Get the handler function for a ToolAPI command.
    ...
    """
    logger.info(
        f"Getting handler for ToolAPI command: {command}",
        component="toolapi",
        operation="get_handler"
    )
    # Log dictionary ID at lookup time
    logger.info(f"Checking _command_handlers ID at lookup: {id(_command_handlers)}", component="toolapi", operation="get_handler")
    return _command_handlers.get(command)


async def handle_request(request: ToolAPIRequest) -> ToolAPIResponse:
    """Handle a ToolAPI request.
    
    This function dispatches the request to the appropriate handler
    based on the command.
    
    Args:
        request: ToolAPI request to handle
    ...
    # Log request
    logger.info(
        f"Processing ToolAPI request: {request.command}",
        component="toolapi",
    ...
    if handler is None:
        ...
        if request.command == ToolAPICommandType.INFO:
            return handle_info_command(request, start_time)
        elif request.command == ToolAPICommandType.STATUS:
            return handle_status_command(request, start_time)
        elif request.command == ToolAPICommandType.LIST_TOOLS:
            return handle_list_tools_command(request, start_time)
        elif request.command == ToolAPICommandType.CANCEL:
            # Not implemented yet
            return create_error_response(
                request_id=request.request_id,
                command=request.command,
    ...
    try:
        ...
        logger.warning(
            f"ToolAPI request timed out after {request.timeout}s: {request.command}",
            component="toolapi",
            operation="handle_request",
        ...
        logger.error(
            f"Error handling ToolAPI request: {str(e)}",
            component="toolapi",
            operation="handle_request",
            exception=e,
            context={"request_id": request.request_id}
...
def handle_info_command(request: ToolAPIRequest, start_time: float) -> ToolAPIResponse:
    ...
    info = {
        ...
        "available_commands": list(ToolAPICommandType),
...
def handle_status_command(request: ToolAPIRequest, start_time: float) -> ToolAPIResponse:
...
def handle_list_tools_command(request: ToolAPIRequest, start_time: float) -> ToolAPIResponse:
    ...
    tools = {
        "core": [
            {"name": ToolAPICommandType.RIPGREP_SEARCH, "description": "Search files using ripgrep"},
            {"name": ToolAPICommandType.AWK_PROCESS, "description": "Process text with AWK"},
            {"name": ToolAPICommandType.JQ_QUERY, "description": "Query JSON with jq"},
            {"name": ToolAPICommandType.SQLITE_QUERY, "description": "Query SQLite database"},
            {"name": ToolAPICommandType.HTML_PROCESS, "description": "Process HTML content"},
            {"name": ToolAPICommandType.PDF_EXTRACT, "description": "Extract text from PDF"},
            {"name": ToolAPICommandType.TABLE_PROCESS, "description": "Process tabular data"},
            {"name": ToolAPICommandType.SEMANTIC_SEARCH, "description": "Semantic search"},
        ],
        "composite": [
            {"name": ToolAPICommandType.PARALLEL_SEARCH, "description": "Search with multiple patterns in parallel"},
            {"name": ToolAPICommandType.RECURSIVE_REFINE, "description": "Recursively refine search results"},
            {"name": ToolAPICommandType.CONTEXT_EXTRACT, "description": "Extract context around matches"},
            {"name": ToolAPICommandType.PATTERN_ANALYZE, "description": "Analyze patterns in text"},
            {"name": ToolAPICommandType.FILENAME_DISCOVER, "description": "Discover filename patterns"},
            {"name": ToolAPICommandType.STRUCTURE_ANALYZE, "description": "Analyze document structure"},
            {"name": ToolAPICommandType.STRUCTURE_SEARCH, "description": "Search based on structure"},
            {"name": ToolAPICommandType.DIFF_GENERATE, "description": "Generate diff between texts"},
            {"name": ToolAPICommandType.REGEX_GENERATE, "description": "Generate regex for patterns"},
            {"name": ToolAPICommandType.DOCUMENT_PROFILE, "description": "Create document profile"},
            {"name": ToolAPICommandType.SEMANTIC_SEARCH, "description": "Semantic search"},
        ],
        "analysis": [
            {"name": ToolAPICommandType.CODE_ANALYZE, "description": "Analyze code"},
            {"name": ToolAPICommandType.DOCUMENT_EXPLORE, "description": "Explore document collection"},
            {"name": ToolAPICommandType.METADATA_EXTRACT, "description": "Extract metadata"},
            {"name": ToolAPICommandType.CORPUS_MAP, "description": "Map document corpus"},
            {"name": ToolAPICommandType.COUNTERFACTUAL_ANALYZE, "description": "Analyze counterfactuals"},
            {"name": ToolAPICommandType.STRATEGY_COMPILE, "description": "Compile search strategy"},
        ],
        "meta": [
            {"name": ToolAPICommandType.INFO, "description": "Get server information"},
            {"name": ToolAPICommandType.STATUS, "description": "Get server status"},
            {"name": ToolAPICommandType.CANCEL, "description": "Cancel a request"},
            {"name": ToolAPICommandType.LIST_TOOLS, "description": "List available tools"},
            {"name": ToolAPICommandType.LIST_STRATEGIES, "description": "List available strategies"},
            {"name": ToolAPICommandType.LIST_TEMPLATES, "description": "List available templates"},
        ],
    }
...
def command_handler(command: str):
    """Decorator to register a function as a ToolAPI command handler.
...
def initialize_handlers():
    ...
    # Register core tools
    register_command_handler(ToolAPICommandType.RIPGREP_SEARCH, handle_ripgrep_search)
    register_command_handler(ToolAPICommandType.JQ_QUERY, handle_jq_query)
    register_command_handler(ToolAPICommandType.AWK_PROCESS, handle_awk_process)
    register_command_handler(ToolAPICommandType.SQLITE_QUERY, handle_sqlite_query)
    register_command_handler(ToolAPICommandType.PDF_EXTRACT, handle_pdf_extract)
    register_command_handler(ToolAPICommandType.TABLE_PROCESS, handle_table_process)
    register_command_handler(ToolAPICommandType.HTML_PROCESS, handle_html_processor)
    
    # Register composite operations
    register_command_handler(ToolAPICommandType.PARALLEL_SEARCH, handle_parallel_search)
    register_command_handler(ToolAPICommandType.CONTEXT_EXTRACT, handle_context_extract)
    
    # Register analysis tools
    register_command_handler(ToolAPICommandType.CODE_ANALYZE, handle_code_analyze)
    register_command_handler(ToolAPICommandType.DOCUMENT_EXPLORE, handle_document_explore)
    register_command_handler(ToolAPICommandType.STRATEGY_COMPILE, handle_strategy_compile)
    
    # Register evolution tools
    register_command_handler(ToolAPICommandType.PATTERN_ANALYZE, handle_pattern_analyze)
    register_command_handler(ToolAPICommandType.DOCUMENT_PROFILE, handle_document_profile)
    register_command_handler(ToolAPICommandType.SEMANTIC_SEARCH, handle_semantic_search)
    
    # Register structure search handler
    register_command_handler(ToolAPICommandType.STRUCTURE_SEARCH, handle_structure_search)
    ...
    logger.info(f"_command_handlers ID after init: {id(_command_handlers)}", component="toolapi", operation="register_handlers")
    
    # Original logging (slightly redundant now but keep for consistency)
    logger.info(
        "Registered ToolAPI command handlers",
        component="toolapi",
        operation="register_handlers",
        context={
            "handler_count": len(_command_handlers),
...
async def handle_pattern_analyze(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if action == "list_categories":
        ...
        try:
            ...
            logger.error(f"TypeError in analyze_pattern: {e}", component="toolapi", operation="pattern_analyze")
            return {
                "status": "error", 
                "error": {
            ...
            logger.error(f"Error in analyze_pattern: {e}", component="toolapi", operation="pattern_analyze")
            import traceback
            return {
                "status": "error", 
                "error": {
...
async def handle_sqlite_query(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        from tsap.toolapi.models import SqliteQueryParams
...
async def handle_table_process(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Convert the result Pydantic model to a dictionary for the ToolAPI response
    ...
    try:
        ...
        for path in paths:
            if not os.path.exists(path):
                logger.warning(f"File not found: {path}", component="toolapi", operation="structure_search")
            else:
                logger.info(f"File exists: {path}", component="toolapi", operation="structure_search")
        ...
        try:
            ...
            logger.error(f"Error in structure search: {e}", component="toolapi")
            import traceback
            logger.error(traceback.format_exc(), component="toolapi")
            return {"status": "error", "error": {"message": str(e)}}
    except Exception as e:
        logger.error(f"Error in structure search request: {e}", component="toolapi")
        import traceback
        logger.error(traceback.format_exc(), component="toolapi")
